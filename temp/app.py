...
def run_twitter_automation(tweet_text):
    ...
    try:
        ...
        python_executable = os.path.join("E:\\FeedingTrends", "venv39", "Scripts", "python.exe")
        cmd = [python_executable, 'twitter_automation.py', temp_file_path]
...
def post_tweet():
    ...
    return jsonify({"status": "processing", "message": "Tweet automation started"})
...
def tweet_status():
    try:
        ...
        return jsonify({"status": "PROCESSING: Initializing..."})
    except Exception as e:
        logger.error(f"Error reading status: {e}")
        return jsonify({"status": f"FAILED: Error reading status - {str(e)}"})
...
def method_not_allowed(e):
    logger.error(f"Method not allowed for {request.url} - Method: {request.method}")
    return redirect(url_for('index'))
